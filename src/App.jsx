...
const HomeTab = ({ appointments, patients, doctorInfo, onAddAppointment, onOpenCalendar, onUpgrade }) => {
  ...
  // 🌟 FIX 1: backgroundSize adaptado a "contain" e backgroundColor escuro para encaixe perfeito
  const bannerStyle = doctorInfo.bannerImage ? { backgroundImage: `url(${doctorInfo.bannerImage})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: 'center', backgroundColor: '#020617' } : {};
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      <div className={`p-8 rounded-[40px] border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-500 ${!doctorInfo.bannerImage ? 'bg-gradient-to-br from-indigo-700 to-black' : ''}`} style={bannerStyle}>
        {/* Opacidade reduzida para ver a foto claramente */}
        {doctorInfo.bannerImage && <div className="absolute inset-0 bg-black/40" />}
        ...
        {/* 🌟 FIX 2: O LOGÓTIPO INFERIOR DIREITO FOI COMPLETAMENTE REMOVIDO DAQUI */}
      </div>
...
const PatientProfile = ({ patient, doctorInfo, onBack, onAddHistory, onDelete, onSchedule }) => {
  ...
  const generateLocalAssistant = () => {
    if (!patient.histories || patient.histories.length === 0) { setSum("Registra ajustes primero para obtener sugerencias."); return; }
    setLoadingIA(true);
    setTimeout(() => { setSum("💡 Análisis:\n\n• CERVICAL: Revisar Atlas/Axis. Sugerido Diversified.\n• LUMBAR: Descartar compresión radicular. Sugerido Gonstead.\n• PELVIS: Excelente respuesta con bloques SOT."); setLoadingIA(false); }, 600);
  };
      ...
      <div className="bg-slate-900/80 rounded-[40px] border border-cyan-400/20 shadow-xl overflow-hidden relative transition-all duration-500">
        {!isEditing ? (
          <div className="relative animate-fade-in">
            {/* 🌟 FIX 1 (Perfil): object-contain para não cortar a imagem de fundo nesta vista */}
            <div className="h-36 w-full bg-slate-950 relative border-b border-white/10">
               {doctorInfo.bannerImage ? <img src={doctorInfo.bannerImage} className="w-full h-full object-contain opacity-90" alt="Fondo" /> : <div className="w-full h-full bg-gradient-to-br from-cyan-900/40 to-indigo-900/40" />}
            </div>
          ...
          <div className="p-8 space-y-6 animate-fade-in bg-indigo-950/20">
             ...
             <div className="border-b border-white/10 pb-6 mb-6">
               ...
               <div className="flex flex-col gap-4">
                 {/* 🌟 FIX 1 (Perfil): object-contain para a imagem de fundo na edição também */}
                 <div className={`w-full h-32 rounded-[20px] flex items-center justify-center border-2 border-dashed relative overflow-hidden ${isLocked ? 'border-slate-700 bg-slate-900/50' : 'border-cyan-500/50 bg-slate-950'}`}>
                   {doctorInfo.bannerImage ? <img src={doctorInfo.bannerImage} alt="Fondo" className="w-full h-full object-contain opacity-90" /> : <ImageIcon className={`w-8 h-8 ${isLocked ? 'text-slate-700' : 'text-cyan-500/50'}`} />}
